...
fn real_main() -> Result<i32, std::io::Error> {
    ...
    let opts = Command::new("sk")
        ...
        .arg(Arg::new("nth").long("nth").short('n').action(ArgAction::Append).allow_hyphen_values(true))
        .arg(Arg::new("with-nth").long("with-nth").action(ArgAction::Append).allow_hyphen_values(true))
    ...
    let preview_window_joined = values_of(&opts, "preview-window").map(|x| x.join(":"));
    options.preview_window = preview_window_joined.as_deref();

    //------------------------------------------------------------------------------
    // initialize collector
    let item_reader_option = SkimItemReaderOption::default()
        .ansi(is_present(&opts, "ansi"))
        .delimiter(last_of(&opts, "delimiter").unwrap_or(""))
        .with_nth(last_of(&opts, "with-nth").unwrap_or(""))
        .nth(last_of(&opts, "nth").unwrap_or(""))
        .read0(is_present(&opts, "read0"))
        .show_error(is_present(&opts,"show-cmd-error"))
        .build();
    ...
    let fz_query_histories = last_of(&opts, "history");
    let cmd_query_histories = last_of(&opts, "cmd-history");
    ...
    let pre_select_n: Option<usize> = last_of(&opts, "pre-select-n").and_then(|s| s.parse().ok());
    let pre_select_pat = last_of(&opts, "pre-select-pat");
    let pre_select_items: Option<Vec<String>> = opts.get_many::<String>("pre-select-items").map(|vals| vals.flat_map(|m| m.split('\n')).map(|s|s.to_string()).collect());
    let pre_select_file = last_of(&opts, "pre-select-file");
    ...
    let bin_options = BinOptionsBuilder::default()
        .filter(last_of(&opts, "filter"))
        .print_query(is_present(&opts, "print-query"))
        .print_cmd(is_present(&opts, "print-cmd"))
        .output_ending(if is_present(&opts, "print0") { "\0" } else { "\n" })
        .build()
        .expect("");
    ...
    //------------------------------------------------------------------------------
    // filter mode
    if values_of(&opts, "filter").is_some() {
    ...
    if values_of(&opts, "expect").is_some() {
    ...
    if let Some(file) = fz_query_histories {
        let limit = last_of(&opts, "history-size")
    ...
    if let Some(file) = cmd_query_histories {
        let limit = last_of(&opts, "cmd-history-size")
            .and_then(|size| size.parse::<usize>().ok())
            .unwrap_or(DEFAULT_HISTORY_SIZE);
...
fn parse_options(options: &ArgMatches) -> SkimOptions<'_> {
    SkimOptionsBuilder::default()
        .color(last_of(options, "color"))
        .min_height(last_of(options, "min-height"))
        .no_height(is_present(options, "no-height"))
        .height(last_of(options, "height"))
        .margin(last_of(options, "margin"))
        .preview(last_of(options, "preview"))
        .cmd(last_of(options, "cmd"))
        .query(last_of(options, "query"))
        .cmd_query(last_of(options, "cmd-query"))
        .interactive(is_present(options, "interactive"))
        .prompt(last_of(options, "prompt"))
        .cmd_prompt(last_of(options, "cmd-prompt"))
        .bind(values_of(options, "bind").unwrap_or_default())
        .expect(values_of(options, "expect").map(|x| x.join(",")))
        .multi(if is_present(options, "no-multi") {
            false
        } else {
            is_present(options, "multi")
        })
        .layout(last_of(options, "layout").unwrap_or(""))
        .reverse(is_present(options, "reverse"))
        .no_hscroll(is_present(options, "no-hscroll"))
        .no_mouse(is_present(options, "no-mouse"))
        .no_clear(is_present(options, "no-clear"))
        .no_clear_start(is_present(options, "no-clear-start"))
        .tabstop(last_of(options, "tabstop"))
        .tiebreak(values_of(options, "tiebreak").map(|x| x.join(",")))
        .tac(is_present(options, "tac"))
        .nosort(is_present(options, "no-sort"))
        .exact(is_present(options, "exact"))
        .regex(is_present(options, "regex"))
        .delimiter(last_of(options, "delimiter"))
        .inline_info(is_present(options, "inline-info"))
        .header(last_of(options, "header"))
        .header_lines(
            last_of(options, "header-lines")
                .map(|s| s.parse::<usize>().unwrap_or(0))
                .unwrap_or(0),
        )
        .layout(last_of(options, "layout").unwrap_or(""))
        .algorithm(FuzzyAlgorithm::of(last_of(options, "algorithm").unwrap()))
        .case(match last_of(options, "case") {
            Some("smart") => CaseMatching::Smart,
            Some("ignore") => CaseMatching::Ignore,
            _ => CaseMatching::Respect,
        })
        .keep_right(is_present(options, "keep-right"))
        .skip_to_pattern(last_of(options, "skip-to-pattern").unwrap_or(""))
        .select1(is_present(options, "select-1"))
        .exit0(is_present(options, "exit-0"))
        .sync(is_present(options, "sync"))
        .no_clear_if_empty(is_present(options, "no-clear-if-empty"))
        .build()
        .unwrap()
}
...
fn last_of<'a>(matches: &'a ArgMatches, id: &str) -> Option<&'a str> {
    matches
        .get_many::<String>(id)
...
fn values_of<'a>(matches: &'a ArgMatches, id: &str) -> Option<Vec<&'a str>> {
    matches.get_many::<String>(id).map(|x| x.map(|x| x.as_str()).collect())
}

fn is_present(matches: &ArgMatches, id: &str) -> bool {
